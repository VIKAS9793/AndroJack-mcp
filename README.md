...
Install the extension in VS Code, or use the npm package directly in other MCP clients. For this release, the wrapper launches `npx -y androjack-mcp@1.6.1` so Marketplace installs resolve to the exact reviewed MCP build. The core product remains the same: one AndroJack MCP server, distributed through multiple channels.
