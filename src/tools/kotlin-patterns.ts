...
const KOTLIN_PATTERNS: PatternEntry[] = [
  // ── Coroutines ─────────────────────────────────────────────────────────────
  {
...
function findPattern(query: string): PatternEntry | undefined {
  const lower = query.toLowerCase();
  return KOTLIN_PATTERNS.find(
    (p) =>
...
export async function kotlinBestPractices(topic: string): Promise<string> {
  if (!topic || topic.trim().length < 2) {
    return (
      `## Available Kotlin/Android Patterns\n\n` +
  ...
  if (pattern) {
    return (
      `## Kotlin Best Practice: ${pattern.name}\n\n` +
  ...
  try {
    ...
    return (
      `## Kotlin Best Practice: "${sanitized}"\n\n` +
