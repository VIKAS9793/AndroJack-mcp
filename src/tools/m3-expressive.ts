...
interface TopicEntry { keywords: string[]; content: string; }
...
function findTopic(query: string): string | null {
  const lower = query.toLowerCase();
  for (const topic of TOPICS) {
    if (topic.keywords.some(k => lower.includes(k))) return topic.content;
  }
  return null;
}
...
export async function material3Expressive(topic: string): Promise<string> {
