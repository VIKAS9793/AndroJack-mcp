...
type PermType = "normal" | "dangerous" | "signature" | "special" | "removed";
...
function findPermission(query: string): [string, PermEntry] | null {
  const q = query.toUpperCase().replace(/^android\.permission\./i, "").replace(/\s+/g, "_");
  for (const [key, entry] of Object.entries(PERMISSIONS)) {
    if (key.toUpperCase() === q) return [key, entry];
  }
  for (const [key, entry] of Object.entries(PERMISSIONS)) {
    if (key.toUpperCase().includes(q) || q.includes(key.toUpperCase())) return [key, entry];
  }
  return null;
}

function formatEntry(key: string, entry: PermEntry): string {
  const emoji: Record<PermType, string> = {
...
export async function androidPermissionAdvisor(query: string): Promise<string> {
  ...
  if (found) return formatEntry(found[0], found[1]);
