...
export async function androidCodeValidator(
  code: string,
  languageHint?: string,
  minSdk?: number,
  targetSdk?: number
  ...
  const language  = detectLanguage(code, languageHint) as Language;
  const ctx: ValidationContext = { language, minSdk, targetSdk };
