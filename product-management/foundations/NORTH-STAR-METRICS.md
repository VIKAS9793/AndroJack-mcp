...
> *The number of Android code blocks that passed through `android_code_validator`
> (Tool 21) and received a PASS or WARN verdict — meaning they were grounded in
> official documentation and cleared of the 22 rule classes before reaching the developer.*
...
| **Rule coverage breadth** | Number of rule classes in validator | 22 (v1.5.0) → 26 (v1.6.3) | More rules = more ground covered |
...
| **Validator false-negative rate** | Known deprecated API patterns missed by the 22 rules | Any new miss added to backlog immediately |
...
### Opt-In Telemetry (v1.6.3 target)
...
v1.6.3 will offer **opt-in telemetry** with explicit consent:
...
| Health metrics | Continuous (automated alerting in v1.6.3) | Engineering |
...
|---|---|---|
| North Star | Unknown — no telemetry | Establish via opt-in in v1.6.3 |
