...
function parseStacktrace(stacktrace) {
    ...
    let errorLine = lines.find((l) => /^(java\.|android\.|kotlin\.|androidx\.|com\.|Caused by:)/.test(l)) ?? lines[0] ?? stacktrace.slice(0, 200);
    ...
    // Top at-frame
    const topFrame = lines.find((l) => l.startsWith("at ") && !l.includes("reflect.")) ?? "";
 ...
 */
async function searchError(parsed) {
 ...
 */
export async function androidDebugger(stacktrace) {
    ...
    const header = `## AndroJack Error Debugger\n\n` +
        `### Parsed Error\n` +
