...
function findByApiLevel(level) {
    return API_LEVELS.find((e) => e.apiLevel === level);
}
function findByName(name) {
    const lower = name.toLowerCase();
    return API_LEVELS.find((e) => e.versionName.toLowerCase().includes(lower) ||
 ...
 * common pitfalls.
 */
function getMinSdkWarnings(minSdk) {
    const warnings = [];
    if (minSdk < 26)
        warnings.push("⚠️ API < 26: Notification channels not available (required for foreground services on Android 8+)");
    if (minSdk < 23)
        warnings.push("⚠️ API < 23: Runtime permissions not available — you must handle permission dialogs manually");
    if (minSdk < 21)
        warnings.push("⚠️ API < 21: Material Design components and VectorDrawable support limited");
    if (minSdk >= 21)
        warnings.push("✅ API 21+: Jetpack Compose minimum supported");
    if (minSdk >= 23)
        warnings.push("✅ API 23+: Safe to use runtime permissions API directly");
    if (minSdk >= 26)
        warnings.push("✅ API 26+: Notification channels required and available");
    return warnings;
}
/**
 * Core handler for android_api_level_check tool.
 */
export function androidApiLevelCheck(input) {
    ...
    if (!isNaN(numeric)) {
        ...
        if (!entry) {
            const closest = API_LEVELS.reduce((prev, curr) => Math.abs(curr.apiLevel - numeric) < Math.abs(prev.apiLevel - numeric) ? curr : prev);
            return (`## AndroJack API Level Check\n**Input:** API ${numeric}\n\n` +
    ...
    // Full table fallback
    const table = `## Android API Level Reference\n\n` +
        `| Version | Codename | API | Year | Market Share |\n|---|---|---|---|---|\n` +
        API_LEVELS.map((e) => `| ${e.versionName} | ${e.codename} | ${e.apiLevel} | ${e.releasedYear} | ${e.marketShareApprox ?? "—"} |`).join("\n") +
