 ...
 */
async function fetchFromDomain(query, domain) {
    try {
 ...
 */
export async function androidOfficialSearch(query) {
    ...
    const results = await Promise.allSettled(domains.map((d) => fetchFromDomain(sanitizedQuery, d)));
    const formatted = results
        ...
        .filter(Boolean)
        .join("\n\n---\n\n");
    return (`## AndroJack Official Search Results\n` +
