...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (a === "--cache-ttl" && args[i + 1]) {
            cacheTtl = parseInt(args[++i], 10);
        }
        if (a.startsWith("--cache-ttl=")) {
            cacheTtl = parseInt(a.split("=")[1], 10);
        }
        if (a === "--no-cache") {
            noCache = true;
        }
...
// ── Banner ────────────────────────────────────────────────────────────────────
function printBanner(host, port, noCache) {
    ...
    console.error("");
    console.error(chalk.cyan(JSON.stringify({ mcpServers: { androjack: { httpUrl: url, timeout: 30000 } } }, null, 4).split("\n").map(l => "  " + l).join("\n")));
    console.error("");
...
async function main() {
    ...
    if (host !== "127.0.0.1" && !noCache) {
        setInterval(() => {
            ...
            process.stderr.write(`AndroJack cache stats — size:${s.size} hits:${s.hits} misses:${s.misses} ` +
