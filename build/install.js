    ...
    return [
        ...
        // ── AWS Kiro IDE ─────────────────────────────────────────────────────────
        {
            ...
            oneClickUrl: (() => {
                ...
                const config = encodeURIComponent(JSON.stringify({ command: "npx", args: ["-y", "androjack-mcp"], disabled: false, autoApprove: [] }));
        ...
        // ── Google Antigravity IDE ────────────────────────────────────────────
        {
            ...
            configKey: "mcpServers",
            format: "standard",
            notes: "After saving: Antigravity Agent pane → '...' → MCP Servers → Manage → Refresh.",
        },
        // ── JetBrains (Android Studio / IntelliJ) ────────────────────────────────
        {
            ...
            configKey: "mcpServers",
            format: "standard",
            notes: "Or add manually: Android Studio → Settings → Tools → AI Assistant → MCP Servers → +",
        },
    ];
}
// ── Config helpers ──────────────────────────────────────────────────────────
...
function mergeConfig(existing, target) {
    const key = target.configKey;
    const serverBlock = target.format === "vscode" ? SERVER_CONFIG_VSCODE : SERVER_CONFIG_STANDARD;
...
function installToPath(configPath, target) {
    try {
...
// ── IDE Detection ────────────────────────────────────────────────────────────
function detectInstalledIdes(targets) {
...
function printBanner() {
    ...
    console.log(chalk.bold("  MCP Installer") +
        chalk.dim("  ·  Fixes: ") +
        fixes.join(chalk.dim(" · ")));
    console.log(chalk.dim("  The Jack of All Android Trades\n"));
}
// ── Status table ──────────────────────────────────────────────────────────────
function printStatusTable(targets) {
    ...
    if (!subcommand || subcommand === "install") {
        ...
        if (ideFlag) {
            ...
            if (result.success) {
                ...
                if (target.notes)
                    console.log(chalk.dim(`     → ${target.notes}`));
            }
            else {
                legacyFail(result.message);
            }
            if (target.oneClickUrl)
                console.log(chalk.yellow(`     One-click install: ${target.oneClickUrl}`));
            return;
        }
        // ── --auto ──────────────────────────────────────────────────────────────
        if (autoFlag) {
        ...
        if (!hasTty()) {
            console.log(chalk.yellow("  ⚠  No interactive terminal detected.") +
        ...
        if (mode === "auto") {
            ...
            clack.log.info(`Detected: ${detected.map((t) => chalk.cyan(t.name)).join(", ")}`);
            // Y/N confirm
            ...
            for (const target of detected) {
                ...
                if (result.success) {
                    s.succeed(chalk.green(`${target.name}`) + chalk.dim(` → ${result.path}`));
                    if (target.notes)
                        clack.log.info(chalk.dim(target.notes));
                }
                else {
                    s.fail(chalk.red(`${target.name}: ${result.message}`));
                }
        ...
        if (mode === "pick") {
            ...
            const selected = targets.filter((t) => chosen.includes(t.id));
            // Y/N confirm
            const confirmed = await clack.confirm({
            ...
            for (const target of selected) {
                ...
                if (result.success) {
                    ...
                    if (target.notes)
                        clack.log.info(chalk.dim(target.notes));
                    if (target.oneClickUrl)
                        clack.log.info(chalk.yellow(`One-click: ${target.oneClickUrl}`));
                }
                else {
                    s.fail(chalk.red(`${target.name}: ${result.message}`));
                }
        ...
        if (mode === "snippet") {
            ...
            clack.note(chalk.bold("Standard MCP config (Claude, Cursor, Windsurf, Kiro, JetBrains, Antigravity):\n") +
                chalk.cyan(JSON.stringify(snippet, null, 2)) +
                chalk.bold("\n\nVS Code .vscode/mcp.json:\n") +
                chalk.cyan(JSON.stringify(vsSnippet, null, 2)), "Manual Config Snippets");
