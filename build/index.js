...
server.registerTool("android_official_search", {
    ...
    description: "REQUIRED FIRST STEP. Searches developer.android.com, kotlinlang.org, and source.android.com " +
    ...
    inputSchema: {
        query: z
            ...
            .min(2)
            .max(200)
            .describe("Search query — be specific. E.g. 'Compose LazyColumn performance', 'WorkManager constraints'."),
    },
    annotations: {
        readOnlyHint: true,
    },
...
server.registerTool("android_component_status", {
    ...
    description: "Checks whether an Android/Kotlin API, class, or library is stable, deprecated, or removed. " +
    ...
    inputSchema: {
        component_name: z
            ...
            .min(2)
            .max(200)
            .describe("Full or short class name to validate. E.g. 'AsyncTask', 'android.widget.ProgressDialog', 'LiveData'."),
    },
    annotations: {
        readOnlyHint: true,
    },
...
server.registerTool("architecture_reference", {
    title: "Android Architecture Reference",
    description: "Retrieves official Android architecture guide content for a given topic. " +
    ...
    inputSchema: {
        topic: z
            ...
            .min(2)
            .max(200)
            .describe("Architecture topic. E.g. 'mvvm', 'compose', 'hilt', 'modular architecture'. Pass 'list' for all topics."),
    },
    annotations: {
        readOnlyHint: true,
    },
...
server.registerTool("android_debugger", {
    ...
    description: "Parses an Android/Kotlin stacktrace and searches official sources (developer.android.com, " +
    ...
    inputSchema: {
        stacktrace: z
            ...
            .min(10)
            .max(4000)
            .describe("Full Android/Kotlin stacktrace or exception text. Include the exception class and 'at' frames."),
    },
    annotations: {
        readOnlyHint: true,
    },
...
server.registerTool("gradle_dependency_checker", {
    ...
    description: "Live version lookup for any Android, Kotlin, or Jetpack library from Google Maven and Maven Central. " +
    ...
    inputSchema: {
        library_name: z
            ...
            .min(2)
            .max(100)
            .describe("Library name. E.g. 'compose', 'room', 'hilt', 'retrofit', 'kotlin-stdlib', 'coil', 'paging'."),
    },
    annotations: {
        readOnlyHint: true,
    },
...
server.registerTool("android_api_level_check", {
    ...
    description: "Maps Android API levels to version names, codenames, and key feature availability. " +
    ...
    inputSchema: {
        api_level_or_name: z
            ...
            .min(1)
            .max(50)
            .describe("API level number (e.g. '26'), version name (e.g. 'Android 14'), codename (e.g. 'Tiramisu'), or 'all'."),
    },
    annotations: {
        readOnlyHint: true,
    },
...
server.registerTool("kotlin_best_practices", {
    ...
    description: "Returns official Kotlin and Android code patterns with ready-to-use snippets and explicit anti-patterns. " +
    ...
    inputSchema: {
        topic: z
            ...
            .max(200)
            .optional()
            .describe("Pattern name or keyword. E.g. 'coroutines', 'compose state', 'room', 'hilt'. Leave empty to list all."),
    },
    annotations: {
        readOnlyHint: true,
    },
...
server.registerTool("material3_expressive", {
    ...
    description: "Complete reference for Material 3 Expressive (M3E) — Google's design system GA on Android 16 (Sep 2025). " +
    ...
    inputSchema: {
        topic: z
            ...
            .max(200)
            .optional()
            .describe("M3E topic. E.g. 'ButtonGroup', 'MotionScheme', 'MaterialShapes', 'migration', 'wear'. Leave empty to list all topics."),
    },
...
server.registerPrompt("androjack_grounding_gate", {
...
server.registerTool("android_permission_advisor", {
    ...
    description: "Complete Android permissions reference. Returns type (normal/dangerous/special/removed), " +
...
server.registerTool("android_testing_guide", {
    ...
    description: "Complete Android testing reference covering: unit tests (JUnit4, MockK, Turbine, coroutines-test), " +
...
server.registerTool("android_build_and_publish", {
    ...
    description: "Complete Android build pipeline and Play Store publishing reference. " +
...
server.registerTool("android_large_screen_guide", {
    ...
    description: "Complete reference for building Google Play-compliant large screen, tablet, foldable, and ChromeOS apps. " +
...
server.registerTool("android_scalability_guide", {
    ...
    description: "Architecture patterns for Android apps serving millions to billions of users. " +
...
server.registerTool("android_navigation3_guide", {
    ...
    description: "Complete Navigation 3 reference (stable November 2025) — the current official navigation library. " +
...
server.registerTool("android_api36_compliance", {
    ...
    description: "Android 16 compliance reference — mandatory for Google Play by August 2026. " +
...
server.registerTool("android_kmp_guide", {
    ...
    description: "Complete Kotlin Multiplatform reference — shares business logic across Android and iOS. " +
...
server.registerTool("android_ondevice_ai", {
    title: "Android On-Device AI / AICore Guide",
    description: "On-device AI reference — Android 16 AICore and ML Kit Gen AI API. " +
...
server.registerTool("android_play_policy_advisor", {
    ...
    description: "Play Store policy reference — October 2025 changes and ongoing requirements. " +
...
server.registerTool("android_xr_guide", {
    ...
    description: "Android XR SDK reference (Developer Preview 3, December 2025). " +
...
server.registerTool("android_wearos_guide", {
    ...
    description: "Wear OS development reference — Wear OS 5 (API 35), Material 3 Expressive for Wear (stable Aug 2025). " +
